...
--
-- SCD Type 1 semantics are preserved within each run via ROW_NUMBER()
-- deduplication: if the same (taxpayer_id, tax_year) appears multiple times
...
ranked AS (
    -- SCD1: latest filing_date per (taxpayer_id, tax_year) gets rn = 1
    SELECT *,
           ROW_NUMBER() OVER (
